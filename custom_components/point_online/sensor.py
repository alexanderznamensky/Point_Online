...
class PointOnlineSensor(CoordinatorEntity, SensorEntity):
    ...
    def native_value(self):
        ...
        if self.entity_description.key == "due_date":
            _LOGGER.warning(
                "PointOnline due_date RAW for %s: value=%r, type=%s",
                self.unique_id,
                value,
                type(value).__name__,
            )
    ...
    def state(self):
        ...
        if self.entity_description.key == "due_date":
            ...
            if value is not None:
                if isinstance(value, datetime):
                    ...
                    try:
                        ...
                        try:
                            formatted_value = datetime.fromisoformat(
                                value
                            ).strftime("%d.%m.%Y")
                        except ValueError:
                            formatted_value = value
            ...
            _LOGGER.warning(
                "PointOnline due_date STATE for %s: value=%r, type=%s",
                self.unique_id,
                formatted_value,
    ...
    def device_info(self) -> DeviceInfo:
        login = self.coordinator.data.get("login", "unknown") if self.coordinator.data else "unknown"
        account_id = self.coordinator.data.get("account_id", "unknown") if self.coordinator.data else "unknown"
    ...
    def extra_state_attributes(self):
        ...
        if self.entity_description.key == "due_date":
            _LOGGER.warning(
                "PointOnline due_date ATTRS for %s: due_date=%r, last_payment_date=%r, previous_payment_date=%r, last_charge_date=%r, previous_charge_date=%r",
                self.unique_id,
                data.get("due_date"),
                data.get("last_payment_date"),
                data.get("previous_payment_date"),
                data.get("last_charge_date"),
                data.get("previous_charge_date"),
            )
...
#         if self.entity_description.key == "due_date" and value:
